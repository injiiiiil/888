...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class ProximityLinkHost : ConditionalTrait<ProximityLinkHostInfo>,
		ILinkHost, ITick, INotifySold, INotifyOwnerChanged, ISync, INotifyKilled, INotifyActorDisposing
	{
		...
		public virtual void OnLinkCreated(Actor self, Actor clientActor, LinkClientManager client)
		{
			if (Info.OccupyClient || LinkWait <= 0)
				LinkCreated(self, new TraitPair<LinkClientManager>(clientActor, client));
			else
				WaitingClients.Add((new TraitPair<LinkClientManager>(clientActor, client), LinkWait + self.World.WorldTick));
		}

		public virtual void OnLinkRemoved(Actor self, Actor clientActor, LinkClientManager client)
		{
		...
		public virtual bool QueueMoveActivity(Activity moveToLinkHostActivity, Actor self,
			Actor clientActor, LinkClientManager client, MoveCooldownHelper moveCooldownHelper)
		{
		...
		public virtual void QueueLinkActivity(Activity moveToLinkHostActivity, Actor self, Actor clientActor, LinkClientManager client)
		{
			...
			else
			{
				...
				if (!WaitingClients.Any(p => p.Client.Trait == client) && !LinkedClients.Any(p => p.Trait == client))
					OnLinkCreated(self, clientActor, client);
			}
		}
		...
		protected virtual void Tick(Actor self)
		{
			...
			for (var i = 0; i < LinkedClients.Count; i++)
			{
				...
				OnLinkRemoved(self, clientActor, LinkedClients[i].Trait);
				i--;
			}
		}

		protected virtual void LinkCreated(Actor self, TraitPair<LinkClientManager> client)
		{
		...
		public virtual void CancelLink(Actor self)
		{
			...
			while (LinkedClients.Count != 0)
			{
				...
				OnLinkRemoved(self, pair.Actor, pair.Trait);
			}
		}
