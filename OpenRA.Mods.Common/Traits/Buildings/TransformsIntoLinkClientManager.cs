...
namespace OpenRA.Mods.Common.Traits
{
	...
	{
		[CursorReference]
		[Desc("Cursor to display when able to link with target actor.")]
	...
	public class TransformsIntoLinkClientManager : ConditionalTrait<TransformsIntoLinkClientManagerInfo>, IResolveOrder, IOrderVoice, IIssueOrder
	{
		...
		{
			if (order.OrderString == "Link" && CanLinkWith(order.Target.Actor, false))
				return Info.Voice;
			else if (order.OrderString == "ForceLink" && CanLinkWith(order.Target.Actor, true))
				return Info.Voice;
		...
		{
			return !IsTraitDisabled && target.TraitsImplementing<ILinkHost>().Any(host => linkClients.Any(client => client.CanLink(host.GetLinkType)));
		}

		/// <summary>Clone of <see cref="LinkClientManager.CanLinkWith(Actor, bool, bool)"/>.</summary>
		public bool CanLinkWith(Actor target, bool forceEnter)
		{
			...
			return !IsTraitDisabled && target.TraitsImplementing<ILinkHost>()
				.Any(host => linkClients.Any(client => client.CanLinkTo(target, host, forceEnter, true)));
		}
	}
}
