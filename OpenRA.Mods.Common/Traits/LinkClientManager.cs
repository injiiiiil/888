...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class LinkClientManagerInfo : ConditionalTraitInfo, ILinkClientManagerInfo
	{
		...
		[Desc("The pathfinding cost penalty applied for each link client waiting to link with a " + nameof(ILinkHost) + ".")]
		...
		INotifyKilled, INotifyActorDisposing
	{
		...
		public void OnLinkCreated(Actor self, Actor hostActor, ILinkHost host)
		{
			foreach (var client in linkClients)
				client.OnLinkCreated(self, hostActor, host);
		}
		...
		public void OnLinkRemoved(Actor self, Actor hostActor, ILinkHost host)
		{
			foreach (var client in linkClients)
				client.OnLinkRemoved(self, hostActor, host);
		...
		IEnumerable<IOrderTargeter> IIssueOrder.Orders
		{
			get
			{
				yield return new LinkActorTargeter(
					...
					() => requireForceMove,
					LinkingPossible,
					(target, forceEnter) => CanLinkWith(target, forceEnter, true));
			}
		}
		...
		string IOrderVoice.VoicePhraseForOrder(Actor self, Order order)
		{
			...
			if (order.OrderString == "Link" && CanLinkWith(order.Target.Actor, false, true))
				return Info.Voice;
			else if (order.OrderString == "ForceLink" && CanLinkWith(order.Target.Actor, true, true))
				return Info.Voice;
		...
		{
			return !IsTraitDisabled && linkClients.Any(client => client.CanLink(type, forceEnter));
		}
		...
		public bool LinkingPossible(Actor target, bool forceEnter = false)
		{
			return !IsTraitDisabled && target.TraitsImplementing<ILinkHost>()
				.Any(host => linkClients.Any(client => client.CanLink(host.GetLinkType, forceEnter)));
		}

		/// <summary>Can we link to this <paramref name="host"/>.</summary>
		public bool CanLinkTo(Actor hostActor, ILinkHost host, bool forceEnter = false, bool ignoreOccupancy = false)
		{
			return !IsTraitDisabled &&
				linkClients.Any(client => client.CanLinkTo(hostActor, host, forceEnter, ignoreOccupancy));
		}

		/// <summary>Can we link to this <paramref name="target"/>.</summary>
		public bool CanLinkWith(Actor target, bool forceEnter = false, bool ignoreOccupancy = false)
		{
			return !IsTraitDisabled && target.TraitsImplementing<ILinkHost>()
		...
		{
			return linkClients.Where(client => client.CanLink(type, forceEnter));
		}
